...
Build a SaaS rostering, timesheet and payroll-adjacent operations system for small Australian hospitality businesses, with deterministic pay calculations, strong record integrity and tenant-scoped operational controls.

## Deployment model

- Multi-tenant SaaS is the target architecture from the outset, even if early pilots are operationally simple.
- Each customer business is a tenant.
- A tenant may initially operate a single venue, but the product must not depend on "one deployment per business" assumptions.
- Configuration is stored in DB and editable by tenant-authorized users subject to audit and permission rules.
...
2. Tenant, membership and role-based access control.
...
- Marketplace-style self-serve tenant signup and low-touch mass onboarding.
