...
This spec set defines the minimum architectural and governance direction required to turn this project into a local, high-touch, multi-tenant SaaS product for small Australian hospitality businesses.
...
7. Confusing a founder-managed local service model with an excuse to skip SaaS-grade tenant and record boundaries.

## Current project gap summary

The current repository is missing or under-specified in the following areas:

- no tenant model or tenant-scoped data ownership model
...
1. The product will become multi-tenant.
