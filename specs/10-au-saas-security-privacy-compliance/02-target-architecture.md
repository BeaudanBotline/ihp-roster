...
Build the product as a multi-tenant employment operations platform with strong record integrity and privacy boundaries, not as a generic CRUD app with authentication.
...
1. Tenant isolation is explicit in the domain model.
...
4. Access is role-based, tenant-scoped and auditable.
...
8. Early-stage manual operations are acceptable where they reduce complexity, provided they remain tenant-scoped, documented and auditable.

## Domain model changes to make early

### Introduce tenancy now

Add a first-class tenant model before expanding features further.
...
Every employment, roster, timesheet, leave, availability and export record should belong to exactly one tenant.
...
- one person to belong to multiple tenants if required later
...
Permissions should be capability-based and tenant-scoped.
