...
- standardised multi-venue SaaS
- no public self-serve venue creation in the near term

This means the next implementation priority is not broader feature breadth. It is the SaaS pivot work required to avoid locking the product into single-venue/internal-tool assumptions.
...
- **Deliverables:**
  - Add `venues` and `venue_memberships` to `Application/Schema.sql`.
  - Add `venue_id` to venue-owned business records.
  - Add required FKs and composite indexes for common venue-scoped queries.
  - Refresh generated types.
- **Acceptance checks:**
  - Venue-owned core records compile with venue linkage.
  - Common access paths have explicit venue indexes.
...
- **Implementation notes:**
  ...
  - Prefer explicit venue ownership in the schema over controller-only conventions.
- **Completion notes:**
  - Added `venues` table (`id`, `name`, `status` with CHECK constraint for `active|inactive`).
  - Added `venue_memberships` table (`venue_id`, `user_id`, `venue_role` with CHECK for `worker|manager|venue_admin|venue_owner`, `is_active`, `UNIQUE(venue_id, user_id)`).
  - Added `venue_id NOT NULL` FK to all venue-owned tables: `staff`, `pay_levels`, `shift_types`, `slot_names`, `day_names`, `roster_weeks`, `staff_availability`, `leave_requests`, `timesheet_entries`.
  - Converted `venue_config` from global singleton (`is_singleton` + `UNIQUE`) to venue-scoped (`venue_id NOT NULL`, `UNIQUE(venue_id)`).
  - Updated `day_names` uniqueness from `UNIQUE(weekday_index)` to `UNIQUE(venue_id, weekday_index)` and `roster_weeks` from `UNIQUE(week_offset)` to `UNIQUE(venue_id, week_offset)`.
  - Added 9 composite indexes for common venue-scoped access paths.
  - Updated `Application/Fixtures.sql` to seed a default dev venue and wire all fixture data to it.
  - Updated `Test/SchemaSpec.hs` with venue/membership type tests, venue_id field assertions across all venue-owned models, and membership default checks.
  - Fixed `Test/ConflictSpec.hs` mock records to include new `venueId` field.
  - Updated column name roundtrip test to replace `is_singleton` with `venue_id` and `venue_role`.
  - Verification: `regen-types`, `typecheck`, `test` (93 examples, 0 failures), `format` all passed.

### A.2 Current venue resolution and venue-scoped auth helpers
- **Status:** [ ]
- **Goal:** Ensure every authenticated request operates inside an explicit venue context.
...
- **Implementation notes:**
  - For the first few venues, a simple single-venue-per-user assumption is acceptable if documented cleanly.
...
- **Implementation notes:**
  - This step should land immediately after venue context is available so the codebase does not end up half-scoped.
...
- **Completion notes:**
  ...
  - **Superseded by current direction:** This slice reflected the earlier internal-tool model and should now be removed/replaced by Phase A.3 managed venue bootstrap.
